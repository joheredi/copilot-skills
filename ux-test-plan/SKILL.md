...
* `MAIN_MODEL` (optional): model to use for the main orchestrator agent. Default: the **strongest available reasoning model** in the environment (e.g. an Opus-class Claude or top GPT-5.x reasoning model). Override to pin a specific model.
* `SUBAGENT_MODEL` (optional): model to use for parallel sub-agents. Default: a **fast, capable mid-tier model** (e.g. a Sonnet-class Claude or comparable GPT model). Override to balance speed, cost, and capability.
...
# MAIN_MODEL / SUBAGENT_MODEL are optional — omit to auto-select the best
# available models. Set them only to pin specific model IDs, e.g.:
...
> **Model selection policy.** Prefer the **newest, most capable available
> model** for each tier below. Any specific model IDs shown are illustrative
> baselines as of writing, not fixed requirements — if a newer/better model in
> the same tier is available in the current environment, use it. Never pin to
> an older model just because it is named here.

| Role | Tier / default | Rationale |
|------|---------------|-------------------------------------------|
| Main orchestrator | **Strongest available reasoning model** (Opus-class Claude or top GPT-5.x) | Strongest reasoning and synthesis for UX judgment, module prioritization, and aggregating findings across sub-agents |
| Parallel sub-agents | **Fast, capable mid-tier model** (Sonnet-class Claude or comparable GPT) | Fast and capable at following structured instructions, Playwright interaction, and template-driven output; reduces wall-clock time and cost when spawning multiple agents |
...
**Overriding**: Pass `MAIN_MODEL` and/or `SUBAGENT_MODEL` in the invocation payload to pin specific models. Otherwise the skill should select the best currently available model for each tier. If the environment does not support per-agent model selection, all agents will use the session's active model — in that case, prefer the orchestrator-tier model.
